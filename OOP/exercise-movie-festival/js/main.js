...
*/
  ...
  function Program(date) {
    ...
    this.getData = function () {
      ...
      for (var i = 0; i < this.listOfMovies.length; i++) {
        programLength += this.listOfMovies[i].movieLength;
        displayInfo += this.listOfMovies[i].getData() + "\n";
      }
  ...
  function Festival(name, numOfMovies) {
    this.name = name;
    this.listoOfPrograms = [];
    this.maxOfMovies = numOfMovies;
    this.addProgram = function (program) {
      this.listoOfPrograms.push(program);
    };
    this.getData = function () {
      var sumOfTitles = 0;
      var displayInfo = "";
      for (var i = 0; i < this.listoOfPrograms.length; i++) {
        sumOfTitles += this.listoOfPrograms[i].listOfMovies.length;
        displayInfo += this.listoOfPrograms[i].getData() + "\n";
      }
      return name + ", has " + sumOfTitles + " movie titles \n\n" + displayInfo;
    };
  }
  ...
  function createMovie(title, genre, duration) {
    ...
    var movie = new Movie(title, genre, duration);
  ...
  var killBill = createMovie("Kill Bill", "action", 105);
  var oldBoy = createMovie("OldBoy", "horror", 205);
  var film1 = createMovie("film1", "action", 165);
  var gladiator = createMovie("Gladiator", "drama", 123);

  var pinkMovies = createProgram("28.10.2017.");
  var happyMovies = createProgram("29.10.2017.");

  var cinemaCity = new Festival("Cinema City", 25);
  pinkMovies.addMovie(film1);
  pinkMovies.addMovie(killBill);
  happyMovies.addMovie(oldBoy);
  happyMovies.addMovie(gladiator);
  cinemaCity.addProgram(pinkMovies);
  cinemaCity.addProgram(happyMovies);

  console.log(cinemaCity.getData());
})();
